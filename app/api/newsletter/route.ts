...
export async function POST(request: Request) {
  try {
    ...
    if (!name?.trim() || !email?.trim()) {
      return NextResponse.json({ message: "נא למלא שם ומייל." }, { status: 400 })
    }
    ...
    await transporter.sendMail({
      ...
      replyTo: email,
      subject: `הרשמה חדשה לעדכונים - ${name}`,
      text: `שם: ${name}\nמייל: ${email}\n\nהנרשמת ביקשה לקבל עדכונים והטבות.`,
      html: `
        <div style="font-family:Arial,sans-serif;line-height:1.6">
          <h2 style="margin-bottom:12px;">הרשמה חדשה לעדכונים</h2>
          <p><strong>שם:</strong> ${name}</p>
          <p><strong>מייל:</strong> ${email}</p>
          <p>הנרשמת ביקשה לקבל עדכונים והטבות.</p>
        </div>
      `,
    })
    ...
    return NextResponse.json({ message: "אירעה שגיאה בשליחת ההרשמה." }, { status: 500 })
  }
}